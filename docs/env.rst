...
needed for developing `spinnerchief`::
...
interpreter inside the ``bin`` directory, which has `spinnerchief` installed
in it's path:
...
The code for `spinnerchief` lives in ``src/``. Make a change and re-run
